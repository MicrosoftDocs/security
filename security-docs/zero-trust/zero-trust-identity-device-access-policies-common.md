...
> [!IMPORTANT]
> **Enforcement change rolling out March–June 2026:** Conditional Access policies that target *All resources* are enforced for sign-ins requesting only [OIDC scopes](/entra/identity-platform/scopes-oidc#openid-connect-scopes) or a [limited set of directory scopes](/entra/identity/conditional-access/concept-conditional-access-cloud-apps#legacy-conditional-access-behavior-when-an-all-resources-policy-has-a-resource-exclusion), even when resource exclusions are present. Users in affected tenants might receive Conditional Access challenges (such as MFA or device compliance) where previously they were allowed access without enforcement. Review any *All resources* policies that include resource exclusions and validate impacted sign-in flows. For more information, see [Conditional Access behavior change for all resources policies with resource exclusions](/entra/identity/conditional-access/concept-conditional-access-cloud-apps#new-conditional-access-behavior-when-an-all-resources-policy-has-a-resource-exclusion).
