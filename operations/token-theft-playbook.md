...
Learn more: [Token tactics: How to prevent, detect, and respond to cloud token theft](https://www.microsoft.com/security/blog/2022/11/16/token-tactics-how-to-prevent-detect-and-respond-to-cloud-token-theft/)

## Prerequisites

* Access to the Azure AD [sign in](/azure/active-directory/reports-monitoring/concept-sign-ins) and [audit](/azure/active-directory/reports-monitoring/concept-audit-logs) logs for users and service principals
* An account with one of the following Azure AD roles assigned:
...
* Enable the advanced hunting feature, and access to the last seven days of event data
  * See [Connect Office 365 to Microsoft Defender for Cloud Apps](/defender-cloud-apps/connect-office-365)
* Access the Unified Access Log for additional signals
  * See [Forensic artifacts in Office 365 and where to find them](https://techcommunity.microsoft.com/t5/microsoft-security-experts-blog/forensic-artifacts-in-office-365-and-where-to-find-them/ba-p/3634865)
* Azure AD [premium risk detections](/azure/active-directory/identity-protection/concept-identity-protection-risks) in Azure AD Premium 2 (P2) and E5 licenses enable more granular investigation triggers and instruction
* Use a managed authentication configuration with password hash synchronization (PHS), not federated, to access additional signals
...
* See [Microsoft Sentinel integration (Preview)](/defender-cloud-apps/siem-sentinel)
* Office sign-in logs and audit logs
* Configure relevant alerts

Learn more:

* [Connect Azure AD data to Microsoft Sentinel](/azure/sentinel/connect-azure-active-directory)
...
Learn more: Set up Azure AD Identity Protection alerts. See [How To: Export risk data](/azure/active-directory/identity-protection/howto-export-risk-data)
...
Learn more: [Generic SIEM integration](/cloud-app-security/siem)

### Integrate SIEM with Microsoft Graph API

Connect SIEM with the Microsoft Graph Security API.

* **Supported integration options** - write code to connect your application to derive insights:
  * See Microsoft Graph security API overview
  * See Microsoft Graph Security API Samples  
...
* **Connectors** - for the API through SIEM solutions, security orchestration automated response (SOAR), incident tracking and service management (ITSM), reporting, and so on
  * See [List of connectors from Microsoft](/graph/security-integration#list-of-connectors-from-microsoft)
...
The Identity Protection feature has the following triggers:

* **Anomalous token (offline detection)** - atypical token characteristics detected, or a token used from an unfamiliar location. The algorithms detecting this behavior use data from Azure AD with Microsoft 365 properties. This detection indicates if the attacker is replaying the token.
...
* **Attempted access of Primary Refresh Token (PRT)** - in Windows 10 and 11, Microsoft Defender for Endpoint detects suspicious access to PRT and associated artifacts. Detections feed into the Azure AD risk score, which controls resource conditional access. This detection is low-volume and infrequent.
* **Microsoft 365 Defender detections** – integrate Identity Protection and Microsoft 365 Defender to see detections in one portal.
  * By default, the most relevant alerts for the security operation center (SOC) are enabled. For all Azure AD IP risk detections, or to turn off the integration, make change in Microsoft 365 Defender. See [Alert service setting](https://security.microsoft.com/settings/mtp_settings/service_alert_settings).
...
**Before you begin**: Complete and enable the prerequisites. In addition, this playbook assumes Microsoft customers and investigation teams might not have Microsoft 365 E5 or Azure AD Premium P2 license suite available or configured. Therefore, note the provided automation guidance.

For this investigation, it's assumed you have an indication of potential token-theft compromise in:

* A user report
* Azure AD sign-in logs example
...
Learn more: [Incident response overview, Response best practices](incident-response-overview.md#response-best-practices)
...
* In Identity Protection, or in a similar feature, alerts suggest token theft
...
* Affected users receive suspicious emails
  ...
  * See [Investigate malicious email delivered in Microsoft 365](/microsoft-365/security/office-365-security/investigate-malicious-email-that-was-delivered) and [Phishing investigation](incident-response-playbook-phishing.md)
...
If there's indication of phishing or other malicious email, see [Investigate malicious email delivered in Microsoft 365](/microsoft-365/security/office-365-security/investigate-malicious-email-that-was-delivered)
...
Learn about rules in the [Azure AD security operations guide](/azure/active-directory/fundamentals/security-operations-introduction)
...
Use Log Analytics or Sentinel (log sourced in Azure AD) to discover questionable identities and anomalies.
...
> [!NOTE]
> Not all Azure AD activity-generating alerts have a corresponding entry in SigninLogs, as seen with anomalous token detection. We recommended you see other tables, such as **OfficeActivity** and **AuditLogs**.
...
Learn more about advanced hunting in: [CloudAppEvents](/microsoft-365/security/defender/advanced-hunting-cloudappevents-table)

### Malicious actions in AuditLogs, AzureActivity, AzureDevOpsAuditing, and CloudAppEvents

Confirm what the attacker accessed: identity documents, code, repositories, etc. Review the items for sensitive information or hardcoded credentials. See the following SharePoint example.
...
* Microsoft 365 Defender portal:
  ...
  * See [Investigate Microsoft Defender for Endpoint alerts](/microsoft-365/security/defender-endpoint/investigate-alerts#investigate-using-the-alert-story)
* Advanced hunting:
...
Contain the device. See [Take response actions on a device in Microsoft Defender for Endpoint](/microsoft-365/security/defender-endpoint/respond-machine-alerts#contain-devices-from-the-network)

### Data accessed by the attacker

Data loss is data destruction or leakage. Discover what the attacker accessed and the data sensitivity. Investigate SharePoint, OneNote, Azure DevOps (ADO). Rotate credentials.
...
2. Block the user. See [Revoke user access in Azure AD](/azure/active-directory/enterprise-users/users-revoke-access).
3. In Identity Protection, or similar feature, mark relevant accounts as Compromised.
4. Block the attacker IP address.

    > [!TIP]
    > Attackers can use legitimate virtual private networks (VPNs), which can create more risk as they change IP addresses. If you use Cloud Authentication, block the IP address in Defender for Cloud Apps, or Azure AD. If federated, block the IP address at the firewall level in front of the Active Directory Federation Services (ADFS).

5. Enable MFA. See [Enable Azure AD Multi-Factor Authentication](/azure/active-directory/authentication/tutorial-enable-azure-mfa)
6. Enable Identity Protection for user and sign-in risk. See [Risk policies: Azure AD Identity Protection](/azure/active-directory/identity-protection/howto-identity-protection-configure-risk-policies)
7. Determine compromised data: emails, SharePoint, OneDrive, apps. See Microsoft Defender for Cloud Apps, [Activities](/cloud-app-security/activity-filters) filter.
8. Maintain password hygiene. See [Password Guidance](https://www.microsoft.com/research/publication/password-guidance/)
...
Delete credentials added with Azure AD authentication methods Graph AP.
...
Learn more: [Azure AD authentication methods API overview](/graph/api/resources/authenticationmethods-overview)
...
If you enabled Microsoft Purview, scan your environment. Use the All credentials entity definition with the compromised accounts. Roll the identified credentials. See the following section for credential rolling.

Learn more: [All credentials entity definition](/microsoft-365/compliance/sit-defn-all-creds)

### Expire and rotate leaked secrets

Rotate secrets associated with the identified user or device credentials.

* [Revoke user access in Azure AD](/azure/active-directory/enterprise-users/users-revoke-access)
* In the Azure portal for the cloud accounts, reset account passwords
* For hybrid accounts, reset the user password twice See Revoke user access in Azure AD
* In the Azure AD user account, verify devices and MFA are under user control:
...
Use the following articles to learn more about securing identities.
...
Sometimes, it's not possible to discover the root cause. We recommended you complete the investigation to derive the details that can illuminate root cause. After recovery, you can perform more investigative steps to help determine the root cause.

Learn more, [Investigate malicious email delivered in Microsoft 365](/microsoft-365/security/office-365-security/investigate-malicious-email-that-was-delivered)
