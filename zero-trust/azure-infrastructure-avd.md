...
<ol type="a">
  ...
  <li>An Azure Virtual Desktop Control Plane</li>
  <li>An Azure Virtual Desktop Management Plane</li>
...
This reference architecture aligns to the architecture described in the Cloud Adoption Framework described in [Enterprise-scale landing zone for Azure Virtual Desktop](/azure/cloud-adoption-framework/scenarios/wvd/enterprise-scale-landing-zone).

## Logical architecture for Azure Virtual Desktop
...
- Azure subscription for your Azure Virtual Desktop subscription
...
- Storage resource group
  ...
  Resources provided by the Azure Files service, such as private endpoints and data sets for creating session hosts are isolated with a storage resource group.

- Session host VMs resource group

  The virtual machines for the session hosts and an Application Security Group are isolated with a dedicated resource group.
...
This article steps through the following to apply the principles of Zero Trust across the Azure Virtual Desktop reference and logical architecture.  
...
| 3 | Apply Zero Trust to Azure Virtual Desktop storage resources. | Verify explicitly <br> Use least privileged access <br> Assume breach |
| 4 | Apply Zero Trust to hub and spoke Azure Virtual Desktop VNets. |  Verify explicitly <br> Use least privileged access <br> Assume breach |
| 5 | Apply Zero Trust to Azure Virtual Desktop session host VMs. | Verify explicitly <br> Use least privileged access <br> Assume breach |
| 6 | Deploy security, governance, and compliance of Azure Virtual Desktop desktops. | Assume breach |
| 7 | Deploy secure management and monitoring of Azure Virtual Desktop desktops. | Assume breach |

## Step 1. Secure your identities with Zero Trust

To apply Zero Trust to the identities used in Azure Virtual Desktop:
...
- Logically separate critical data with network controls. Such as separate storage accounts for different host pools and other purposes as with MSIX app attach file shares.
- Use Defender for Storage for automated threat protection.

Note: In some designs [Azure NetApp files](/azure/virtual-desktop/create-fslogix-profile-container) is the storage service our choice which is supports FSLogix profiles for Azure Virtual Desktop via an SMB share. Azure NetApp Files provides built-in security features that include [delegated subnet](/azure/azure-netapp-files/azure-netapp-files-delegate-subnet) and [security benchmarks](/security/benchmark/azure/baselines/azure-netapp-files-security-baseline?toc=%2Fazure%2Fazure-netapp-files%2FTOC.json).
...
A spoke VNets isolates the Azure Virtual Desktop workload and contains the session host virtual machines. Implement the steps in [Apply Zero Trust principles to spoke virtual network in Azure](azure-infrastructure-iaas.md) for the spoke  VNet that contain the virtual machines of your session hosts. Isolate different host pool on sperate VNETs together with [NSG](/azure/virtual-network/network-security-groups-overview) with the required URL necessary for Azure Virtual Desktop. Azure Firewall or a network virtual appliance (NVA) firewall can be used to control and restrict  outbound traffic for Azure Virtual Desktop session hosts.

Use the instructions [here](/azure/firewall/protect-azure-virtual-desktop?tabs=azure) to use Azure Firewall to protect session hosts. Force the traffic through the firewall with [User-Defined Route (UDR)](/azure/virtual-network/virtual-networks-udr-overview#user-defined) linked to the Host pool subnet. Review the full list of required [Azure Virtual Desktop URLs](/azure/virtual-desktop/safe-url-list?tabs=azure) to configure your firewall. Azure Firewall provides an Azure Virtual Desktop [FQDN Tag](/azure/firewall/fqdn-tags#current-fqdn-tags) to simplify this configuration.

## Step 5. Apply Zero Trust principles to session hosts
...
Azure Virtual Desktop has built-in advanced security features to protect session hosts. However, there are additional steps to follow to improve the security defenses of your Azure Virtual Desktop environment and session hosts:
...
## Step 7. Configure management and monitoring of Azure Virtual Desktop environment

Management and continuous monitoring are important to ensure that your Azure Virtual Desktop environment are not engaging in malicious behavior. Use [Azure Virtual Desktop Insights](/azure/virtual-desktop/insights) to log data and report diagnostic and usage data.

Additionally:

- Review recommendations from [Azure Advisor for Azure Virtual Desktop](/azure/virtual-desktop/azure-advisor-recommendations).
- Use [Microsoft Intune](/azure/virtual-desktop/management#microsoft-intune) for granular policy management or [group policy management](/windows-server/remote/remote-desktop-services/rds-vdi-recommendations). Create separate OU for different host pools.
...
See these additional articles for Zero Trust deployment plans:
