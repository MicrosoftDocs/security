...
<!-- - **The Event planning site configured with starting point protection**: The site isn't configured in SharePoint access control, so users with site permission can access the site from their managed devices only. --->
...
Defender for Cloud Apps is a valuable tool for configuring protection for compliance regulations. Create specific policies to look for regulated data and configure each policy to take appropriate action.
